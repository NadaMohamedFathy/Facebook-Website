...
$(document).ready(function () { //variables w asma2 el classes
    ...
    arrow.on('click', function () { //btghyar shakl el arrow fl chat
    ...
    sendButton.on('click', function () { //send message
        var msg = textarea.val();
        if (msg !== '') {
            textarea.val('');
            $('.msg-insert').append("<div class='msg-send'>" + msg + "</div>"); //append el msg 3ala el msg w bthoto fl body
            $.ajax({ //Perform an asynchronous HTTP (Ajax) request.
                ...
                success: function () { //A callback function to be executed when Ajax request succeeds.
                }
            });
        }
    });
    ...
    textarea.keypress(function (event) {
        ...
        if (event.keyCode === 13) { //13 is the key code of enter
            var msg = $this.val();
            if (msg !== '') {
                textarea.val('');
    ...
    like.on('click', function (e) {
        popupWrapper.css("visibility", "visible"); //view message pop-up
...
function startConv(s_email, r_email) { //start conv + get old chat
